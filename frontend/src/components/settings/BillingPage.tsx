...
export const BillingPage = () => {
  ...
  const handleUpgrade = async (plan: string) => {
    ...
    try {
      ...
      const { data } = await api.post(`/payments/create-subscription?plan=${plan}`);
      ...
      const options = {
        ...
        prefill: {
          ...
          email: caFirm?.id, // Note: caFirm ID or email as per your preference
        },
        theme: {
          color: '#2563EB',
        },
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <p className="text-gray-600 mb-8">
        Current Plan: <span className="font-semibold text-blue-600 capitalize">{caFirm?.subscription_plan}</span>
      </p>
...
const PlanCard = ({ title, price, features, onUpgrade, isLoading, isPopular }: any) => (
